...
def load_mysubreddits():
    ...
    return render_template('index.html',
                           subs=get_subscribed_subs(access_token))
...
def get_subscribed_subs(access_token):
    headers = {'Authorization': 'bearer ' + access_token,
               'User-Agent': 'image-viewer by /u/gabranches'}
    response = requests.get('https://oauth.reddit.com/subreddits/'
                            'mine/subscriber/.json?limit=100', headers=headers)
